...
namespace Microsoft.VisualStudio.CodeAnalysis.CodeLens
{
    ...
    internal class ReferenceCodeLensProvider : IAsyncCodeLensDataPointProvider
    {
        ...
        private async Task<Stream> GetConnectionAsync(CancellationToken cancellationToken)
        {
            ...
            var hostGroupId = await _lazyCodeLensCallbackService.Value.InvokeAsync<string>(this, nameof(ICodeLensContext.GetHostGroupIdAsync), arguments: null, cancellationToken).ConfigureAwait(false);
